...
const StoreBookingPatient = ({
    ...
    const { onSubmitHanler, loading, isSuccess, data } = useFormSubmit(
        save,
    ...
    const {
        ...
        isLoading,
        isError,
    } = useQuery<PatientBooking>(
        ["patient", patientRedux?.id],
    ...
    const { selectBox, updateSelectBoxValue, updateSelectBoxOptions } =
        ...
        if (provinces.data && provinces.data.length) {
            ...
            if (dataPatient) {
                updateSelectBoxValue(
                    "province_id",
                    provinces.data,
                    String(dataPatient?.province_id)
                );
            }
        }
    }, [
        ...
        if (districts.data && districts.data.length) {
            ...
            if (dataPatient) {
                updateSelectBoxValue(
                    "district_id",
                    districts.data,
                    String(dataPatient?.district_id)
                );
            }
        }
    }, [
    ...
    useEffect(() => {
        if (isSuccess === true && data && data.booking) {
            if (data.payUrl) {
                ...
                navigate(`${redirectIfSucces}/${paramIdBooking}.html`);
            }
        }
    }, [isSuccess, data]);
    return (
